...
with open("log.txt", "w", encoding='utf8') as log:
    ...
    log.write('Beginning file download with wget module\n')
    ...
    for u in url:
        ...
        log.write('Processing ' + groupName + ' schedule...\n')
    ...
    if storedDataPath not in os.listdir():
        ...
        log.write('No previous updates have been found\n')
        ...
        for c, t in data.items():
            if t == storedData[c]:
                ...
                log.write(c + ': New schedule found!!!\n')
                log.write('Date of publication: ' + t + '\n')
                shutil.move(c + '.pdf', os.path.join(desktop, c + '_' + t.split(' ')[0].replace('/', '.') + '.pdf'))
                newVersionFound = True
    ...
    if newVersionFound:
        ...
        log.write('No changes have been found\n')
